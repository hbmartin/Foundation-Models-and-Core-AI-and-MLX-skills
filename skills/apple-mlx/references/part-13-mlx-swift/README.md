...
|---|---|---|
| "The sample code I found doesn't compile" | [13.1 §1, §2.6](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | It is 2.x. The migration table is there |
| "First build fails and I don't know which products to link" | [13.1 §2–§3](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | Nine products, **three integration styles**; §3.5 decides for you |
| `ModelFactoryError.noModelFactoryAvailable` | [13.1 §4.2](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) · [13.3 §3.3](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/03-fm-bridge-and-guided-generation.md) | You linked `MLXLMCommon` but not `MLXLLM`/`MLXVLM`; the registry uses `NSClassFromString` |
| "cannot find `MLXLanguageModel` in scope" | [13.3 §1](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/03-fm-bridge-and-guided-generation.md) · [13.1 §9.1](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | You are on the 26 SDK. The target compiled to an empty library |
| "It is killed on device with no crash report" | [13.1 §6.7–§6.8](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | Jetsam. `Memory.snapshot()` reports MLX's allocator, not `phys_footprint` |
| "It crashes when the app backgrounds mid-generation" | [13.1 §5.6](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | A Metal command-buffer error on a completion handler Swift cannot catch |
| "The VLM describes a rotated scene" · "VLM prefill OOMs" | [13.1 §7.4, §7.7](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) | EXIF orientation; merged-sequence attention growing as (Σ Lᵢ)² |
| "My 'waiting for first token' spinner never stops" | [13.2 §2.5](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) | The first stream element can be `.toolCall` with **zero** `.chunk` events |
| "The model recites a function call at the user" | [13.2 §7.5](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) | `ToolCallFormat.infer` returned `nil`, the loop assumed `.json` |
| "Output is fluent but subtly worse than it should be" | [13.2 §6.4–§6.5](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) | The chat template is the contract and nothing checks it |
| "I set `kvBits` and memory didn't drop" · "quality collapses mid-generation" | [13.2 §8.5, §9.1](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) | `RotatingKVCache.toQuantized()` is never called; and `#312` |
| "Speculative decoding gives wrong answers on Gemma" | [13.2 §9.2](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) | `#424` — generation continues on tokens that were never emitted |
...
| "`session.prewarm()` does nothing" · "usage is zero" | [13.3 §7.6, §8.8](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/03-fm-bridge-and-guided-generation.md) | A witness that didn't bind; and a deliberately-dropped channel send |
| "I want `@Generable`-style JSON on iOS 17" | [13.3 §9](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/03-fm-bridge-and-guided-generation.md) | `MLXGuidedGeneration` is standalone — no Foundation Models, no 27 floor |
...
**Everyone starts at [13.1 §1–§4](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md).** The 3.x break, the
...
**Read two out of order.** [13.1 §9](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) before you configure
CI, because the 26-versus-27 SDK question decides your build matrix and cannot be papered over with
`@available`. And [13.3 §11](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/03-fm-bridge-and-guided-generation.md) before you pick a
backend, because "does this engine expose logits" is the question that decides whether `@Generable`
works at all.

**Deferrable.** [13.1 §7](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md) unless you ship a VLM;
[13.2 §9–§10](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/02-generation-tools-and-caching.md) unless you enable KV quantization,
...
- **Porting a model architecture to Swift.** `@ModuleInfo`/`@ParameterInfo` conventions,
  ...
  `skills/mlx-swift-lm/references/model-porting.md`. [13.1 §10.4](https://github.com/hbmartin/Foundation-Models-and-Core-AI-and-MLX-skills/blob/main/guides/part-13-mlx-swift/references/01-mlx-swift-lm-in-an-app.md)
