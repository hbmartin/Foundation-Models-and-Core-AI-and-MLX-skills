...
description: "Metal TensorOps and Metal Performance Primitives for hand-written on-device ML kernels on Apple silicon: quantized operands, multiplane quantized tensors, cooperative tensors, and implementing flash attention in Metal."
when_to_use: Use when writing or debugging a Metal compute kernel for machine learning, working with MPP or MTLTensor APIs, hand-rolling an attention kernel, chasing a threadgroup or memory-layout performance cliff, or working out whether a TensorOps surface is 26.x or 27-only.
---
...
|---|---|---|
| "Session 330 promised me scale planes / MX / E8M0" | 11.1 §0.2 · 11.2 §0.3 | Use Xcode 27's multiplane `MTLTensor` API; hand-dequantize only for older targets or custom formats |
| "I read TensorOps is iOS 27 / macOS 27" | 11.1 §1 | The core surface is 26.x; only the newer tensor formats (multiplane, int2/FP4/FP8/E8M0) are 27.0. §1.5 is the deployment-target cheat sheet |
| `no member named 'matmul2d' in namespace 'mpp::tensor_ops'` | 11.1 §2.2 · 11.2 §0.2 | `__HAVE_TENSOR__` is undefined and the header expanded to nothing |
| "I can't find `metal_cooperative_tensor` anywhere in Xcode" | 11.2 §0.1 | It is in the cryptex Metal toolchain. `xcrun -sdk macosx --find metal`; never hardcode the path |
| `static_slice` / `get_mask` won't compile | 11.1 §5.4, §6.4 | Neither exists (`static_slice` toolchain-verified absent, 2026-07-31). Real spellings: templated `slice<…>()` and `is_valid_element` |
| Compile error deep inside `__mutmul2d_detail` | 11.2 §3 · 11.1 §6.3 | The input getters take **element** types; the destination getter takes **operand** types |
| "My K loop returns only the last chunk" | 11.1 §3.5 · 11.2 §5.5 | Descriptor mode defaults to `multiply`. Zero the destination, pass the mode explicitly |
| "Attention quality dropped and I can't find a bug" | 11.2 §6.3 · 11.1 §7.2 | `reduce_rows(…, max)` with three arguments computes `max(0, row)` |
| "Right shape, transposed or scrambled content" | 11.1 §3.4 | `slice()` is `(dim0, dim1)`; the descriptor is `(m, n)`. They are opposite |
| "I want to write a fused attention kernel" | 11.2 §8–§9 | The six steps, then the assembled listing — and §8.7 for what the six steps omit |
| "Correct, but the profiler shows 0% accelerator" | 11.2 §14 | You are on the fallback shader path, or SIMD groups have drifted across K |
| "Wrong numbers from MLX on one tile shape" | 11.2 §13.2 | `tile_matmad_nax` has no `else`; some shapes compile to nothing (upstream PR #3924) |
| "How does this kernel get into a Core AI model?" | 11.2 §12 → Part 8 guide 3 | `TorchMetalKernel`'s `src` is the **body only**; your includes go in `helper_src` |
