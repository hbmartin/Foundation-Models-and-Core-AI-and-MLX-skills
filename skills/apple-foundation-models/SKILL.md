...
| "I hardcoded 4096" | [3.1 §3](references/part-03-context-profiles-agentic/references/01-context-window-and-kv-cache.md#3-reading-the-budget-contextsize-and-the-4096-token-window) | Read `contextSize` at runtime. The macOS host and physical device returned 4,096, while both Simulator hosting modes most recently returned 0. The tool-hosted Simulator had returned 4,096 earlier on the same recorded builds, so treat even a topology-keyed value as observed rather than universal; the 8,192 report remains uncorroborated. |
