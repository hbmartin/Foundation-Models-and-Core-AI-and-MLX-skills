...
| File | Contents |
|---|---|
| `fm-cli-real-machine-evidence.md` | First third-party `fm --help` from a real macOS 27 build — **7 subcommands**, attested flags, and corroboration for `fm serve` alongside the Apple-member statement |
| `wwdc2026-253-music-understanding.md` | Full transcript + 18 verbatim code samples for a framework with **no pre-harvest guide or captured-SDK coverage** |
| `wwdc2026-328-mlx-swift-numerical.md` | Full transcript + 6 code samples; the one ML-track session with no corpus transcript |
| `wwdc2026-8121-ml-ai-group-lab.md` | Apple's published Q&A summaries — documents 4096 for iOS 27 while leaving the alleged device-specific 8192 result uncorroborated, plus stack-layering guidance |
...
### 3. ⭐ Six new transcripts plus two adjacent records — and a method bug that caused the gap

New transcript files retrieved: **328** (MLX Swift numerical computing — the *only* one of the 18
ML-track sessions with no corpus transcript), **253**, **297** (Visual Intelligence), **375**
(Image Playground), **310** (Shortcuts), and **258**. The same pass re-reviewed the existing
**256** transcript and harvested **8121**, the ML & AI Group Lab, as a written Q&A summary because
Apple publishes no caption track for Group Labs.
...
### 4. ⭐ Apple documented the iOS 27 `contextSize` platform value

Apple's published Q&A summary for Group Lab 8121 ch. 0:08:11 records **4096 tokens as the
on-device shared budget** and **32K as PCC's shared budget**. That documents the iOS 27 platform
value without disproving the alleged device-specific 8192 observation, which remains
uncorroborated. The standing advice (read `contextSize` at runtime, never hardcode) is unchanged;
the device test drops in priority rather than disappearing.
...
narrowed it to "the Concurrency library or a sample-local helper"; it is the former. The proposal
requires runtime changes and does **not** back-deploy: on Apple platforms, the standard function
therefore requires both a Swift 6.4 compiler and an OS 27 runtime.
...
On a Swift 6.4 toolchain that is a silent substitution. The compatibility implementation needs its
distinct `withCancellationShieldCompat` name and remains necessary for older compilers or runtimes.
...
| 3 | 4096 recorded as Apple's documented iOS 27 platform value from its Group Lab written Q&A summary; shared input+output budget and tool-definition cost added; the alleged device-specific 8192 result retained as uncorroborated | 17.1, 3.1, 1.1 | ✅ **DONE** |
...
| 7 | Clone fast-forwarded `5ed9981` → `49becc6`. New §7.3.1 on `rollback` / `findJumpForwardString` / `fillBitmask`; new §7.1 on `--clear-coreai-cache` and the pre-clear probe → clear → optional post-clear verification → load measurement pattern; drift table incl. the **`.llmasset` rename** | 7.4, 7.2, 10.3 | ✅ **DONE** |
| 12 | New §13 on the third-party training layer (12 algorithms, QAT, Unsloth-compatible `mlx-tune`); the three 🔴 checked against named project documentation | 12.6 | ✅ **DONE** |
...
8K figure rewritten, `scripts/verify-snippets.sh` (1,359 fences, **0 failures, 0 UNCLASSIFIED**),
...
> the classified TSVs are keyed on `(file, line, anchor, kind)`, so `build-indexes.sh` will refuse
> to build until they are re-keyed. For manual re-keying, match on `(file, anchor, kind)` + ordinal
> within the group. **Watch
