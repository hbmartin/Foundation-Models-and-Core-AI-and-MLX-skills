...
| File | Repo | Description |
|---|---|---|
| `noema-ios.md` | **Frozen Noema 3.5 snapshot** (upstream unavailable 2026-09-16) | Historical community evidence from captured commit `8366103f`, retained for its shipping/ops patterns: six backends, launch memory gating, MoE-plus-mmap residency accounting, pressure hysteresis, verified unloads, background downloads, thermal policy, and Core AI integration. It is no longer cloned or freshness-checked; corroborate before using it as current guidance. |
...
| `issues-community-stack.md` | `apple/python-apple-fm-sdk`, `apple/dnikit`, `1amageek/swift-lm`, frozen Noema 3.5 snapshot, `lucasnewman/mlx2coreai`, `john-rocky/coreai-model-zoo`, `john-rocky/coreai-models` | Issue/PR mining across the community half of the stack, with per-repo activity stats. The Noema portion is a frozen historical capture and no longer participates in refresh jobs. Source of the Python SDK bug list, the `.aimodel` bundle-path version error, the iOS "No such file or directory" JIT failure signature, and the community porting incidents. |
