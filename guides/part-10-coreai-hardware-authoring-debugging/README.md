...
- **Conversion mechanics** — `torch.export`, `TorchConverter`, decomposition tables, externalisation,
  composite ops, the `.aimodel` format: [Part 8](../part-08-coreai-pytorch-conversion/). **Choosing a
  compression recipe** — `Quantizer` vs `KMeansPalettizer`, granularities, calibration, QAT:
  [Part 9](../part-09-coreai-compression-numerics/). Guide 10.1 states only which compression *shapes*
...
- **The Swift runtime** — `AIModel`, `InferenceFunction`, `NDArray`, states, `SpecializationOptions`,
  `AIModelCache`: [Part 7](../part-07-coreai-swift-runtime/); guide 10.3 §11 shows only the integration
  surface an exporter needs. **Custom Metal kernels:** [Part 11](../part-11-metal-and-tensorops/) —
...
- **Consuming a Core AI model behind `LanguageModelSession`**, including the capability and options
  mismatches you inherit: [Part 4](../part-04-beyond-the-built-in-model/). **MLX as a framework**
  rather than as a source checkpoint: [Parts 12](../part-12-mlx-python/) and
  [13](../part-13-mlx-swift/); the `mlx2coreai` bridge in full and the reverse direction:
  [Part 14](../part-14-bridges-between-stacks/).
- **Foundation Models' own Instruments template:** [Part 5](../part-05-prototyping-profiling-non-swift/).
  **Whether the model you shipped is any good:** [Part 6](../part-06-evaluations/). **Background Assets
  delivery and operating a shipped model:** [Part 15](../part-15-shipping-and-operating/). **Moving a
