...
over another backend is [Part 1](../part-01-orientation-and-gating/).
...
- **Authoring a `LanguageModel` conformance and choosing among the five conformers:**
  [Part 4](../part-04-beyond-the-built-in-model/) — `CoreAILanguageModel` is dissected in 7.4 §8 only
  where it differs from the worked examples there. Everything *in front of* the session — `@Generable`,
  `Tool`, streaming, the failure taxonomy — is [Part 2](../part-02-foundation-models-everyday-api/), and
  context management as a discipline is [Part 3](../part-03-context-profiles-agentic/).
